...
class DataAnalysis:
    ...
    def __init__(self, experiment, analyze=True, average=False, num_shots=2**14) -> None:
        """__init__ _summary_
        ...
        if analyze:
            self.retrieve_data(experiment=experiment, average=average)
            self.lda()
            self.countpop()
            self.error_mitiq()  
    ...
    def retrieve_data(self, experiment, average: bool) -> None:
        """retrivedData
        ...
        Returns:
            list: IQ_data ?
        """ 
        experiment_results = experiment.result(timeout=120)
        self.IQ_data = []
    ...
    def error_mitiq(self, data, assign_matrix):
        """errorMitiq 
        ...
        for i in range(len(data)):
            data_mitiq.append(self.dataMitigator(data[i], assign_matrix))
        data_mitiq = np.array(data_mitiq)
        
        return data_mitiq

    def data_mitigator(self):
        ...
        Returns:
            _type_: _description_
        """
        calibration_matrix = np.transpose(self.raw_counted[0:3])
        raw_data = self.raw_counted
        nshots = sum(raw_data)
        
        def fun(x):
            return sum((raw_data - np.dot(calibration_matrix, x)) ** 2)
...
# Fitting functions
def fit_function(self, x_values, y_values, function, init_params):
